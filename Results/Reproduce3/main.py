...
#Modification: Add import and set_seed
import math
import random

def set_seed(seed: int):
    os.environ["PYTHONHASHSEED"] = str(seed)
    #For CUDA matmul determinism (needed when using torch.use_deterministic_algorithms):
...
def lam_schedule_value(lam0: float, t: int, T: int, sched: str) -> float:
    """
    Return λ_t from λ_0 and schedule type.
    - 'none'   : constant λ_0
    - 'cosine' : λ_t = λ_0 * 0.5 * (1 + cos(pi * t / T))
    """
    s = (sched or "none").lower()
    if s == "cosine":
        return float(lam0) * 0.5 * (1.0 + math.cos(math.pi * float(t) / max(1.0, float(T))))
    return float(lam0)
...
#Modification: replace function
'''def extract_sv(W, name, T):
    U,D,V = W.svd()'''

def extract_sv(W: torch.Tensor, topk: int = 5) -> torch.Tensor:
    sv = torch.linalg.svdvals(W)
    sv_sorted, _ = torch.sort(sv, descending=True)
    return sv_sorted[:topk]
...
def solve_nn(data, args, device):
    ...
    for T in range(args.niters):
        #Modification: compute value
        lam_t = lam_schedule_value(args.lam, T, args.niters, getattr(args, "lam_schedule", "none"))
        #
        W_ = W_e2e(model, args)
        Y_ = W_[X_x, X_y]
        ...
        if lam_t > 0:
            if args.reg_norm == "ratio":
                loss += lam_t * (W_.norm('nuc') / (W_.norm('fro') + 1e-12))
            elif args.reg_norm == "nuclear":
                loss += lam_t  * (W_.norm('nuc'))
            elif args.reg_norm == "sc_12":
                svs = extract_sv(W_)
                loss += (lam_t ) * torch.linalg.norm(svs, ord=1/2) 
            elif args.reg_norm == "sc_13":
                svs = extract_sv(W_)
                loss += (lam_t ) * torch.linalg.norm(svs, ord=1/3) 
            elif args.reg_norm == "sc_23":
                svs = extract_sv(W_)
                loss += (lam_t ) * torch.linalg.norm(svs, ord=2/3) 
            elif args.reg_norm == "sc_1223":
                svs = extract_sv(W_)
                loss += (lam_t ) * (torch.linalg.norm(svs, ord=1/2)/torch.linalg.norm(svs, ord=2/3))
            elif args.reg_norm == "sc_1323":
                svs = extract_sv(W_)
                loss += (lam_t ) * (torch.linalg.norm(svs, ord=1/3)/torch.linalg.norm(svs, ord=2/3))
            elif args.reg_norm == "sc_1312":
                svs = extract_sv(W_)
                loss += (lam_t ) * (torch.linalg.norm(svs, ord=1/3)/torch.linalg.norm(svs, ord=1/2))
        ...
        if T % args.log_interval == 0:
            with torch.no_grad():
                ...
                #Replaced with:
                # Clamp only for MovieLens; never clamp for synthetic Gaussian completion
                if args.data == "ml-100k-sample":
                    Y_test = torch.clamp(Y_test, min=1, max=5)
                ...
                train_loss = torch.mean((Y - Y_)**2)
                effrank   = effective_rank(W_).item()
                test_mse  = test_loss.item() #paper plots MSE (“error”) on log scale
                test_rmse = math.sqrt(test_mse) #keep RMSE for backward compatibility


                #commmented out from original and inserted below block: #print(f"depth: {args.depth}, iteration: {T}, test_RMSE: {test_rmse}, erank: {effrank}")
                #singular values (top-1 and 5th) for the current factorized weight W_
                sv = torch.linalg.svdvals(W_)
                sv_sorted, _ = torch.sort(sv, descending=True)
                k = min(5, sv_sorted.numel())
                s1 = float(sv_sorted[0].item()) if k >= 1 else float('nan')
                s5 = float(sv_sorted[4].item()) if k >= 5 else float('nan')
                print(f"depth: {args.depth}, iteration: {T}, test_MSE: {test_mse}, test_RMSE: {test_rmse}, erank: {effrank}, s1: {s1}, s5: {s5}")                
                #finishs of block inserted
...
def main():
    ...
    #Modification: Add
    parser.add_argument("--lam_schedule", type=str, default="none", help="Lambda schedule: {'none','cosine'}. Cosine: lam_t = lam * 0.5 * (1 + cos(pi*t/T)).")
    parser.add_argument('--seed', type=int, default=67, help='global RNG seed')
    #
